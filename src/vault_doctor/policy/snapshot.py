"""文件快照（M2-B）：修复落盘前按会话复制受影响文件，支持一键回滚。

设计取舍（对 DESIGN 6.1 的统一化简化）：不做 git/影子仓库双轨——快照的唯一
职责是回滚本工具自己的写入，按会话复制将被修改的文件即可，git 与非 git
库统一、零外部依赖。copy2 保留 mtime/size，回滚后增量索引指纹自动吻合。
"""
