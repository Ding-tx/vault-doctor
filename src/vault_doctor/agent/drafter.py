"""修复起草器：LLM 提议"改什么"，代码决定"改哪里"。

分工理由：模型不擅长数字符偏移，但擅长判断"把 [[X]] 改成 [[Y]]"。
...
DRAFT_SYSTEM_PROMPT = (
    ...
    "[[算法导论]]），不动行文；链接目标无歧义时用裸文件名（不带路径、不带 .md "
    "后缀，如 [[算法导论]]），仅当裸名会与其他文件撞名时才写相对路径；"
    "reason 必须具体可核对——写明原链接是什么、为什么判定它该改成候选中的那个文件"
    "（如\"[[算法导论2]] 是 [[算法导论]] 改名后未同步的链接\"），"
    "禁止\"修复链接\"\"替换为正确目标\"这类空泛表述；"
    "某条违规确实不该修时给出 reason 并把 old_text 留空；"
    "不修改与违规无关的任何内容。"
)
